<?php 
/*
Template Name: ObjektmallNyVersion
*/ 
	?>
...
<div class="gdl-page-header-area-2" id="gdl-page-header-area">
	...
	<h1 class="gdl-page-title page-title-hemnet gdl-title title-color"><?php echo "$adressRad1" ?><?php if($kind == '510') { ?> - SÅLD<?php } ?></h1>
</div>
<div class="sixteen columns wrapper">
	<div class="ten columns gdl-first "> <!-- 600 -->
		<div class="gdl-column-item"> <!-- gdl-first gdl-last -->
			<div class="gdl-image-frame shortcode-image-left" style="max-width: 100%; float: left; width: 600px; height: 350px; margin-left: -20px; "><a href="<?php echo '/visa-bilder-hemnet.php?objekt=' . $objID; ?>" title="Fantastic Frank Fastighetsmäkleri - <?php echo "$adressRad1" ?>" ><img src="/wp-content/themes/incidental/images/icon/light/content-bg-transparent.png" style="border: none; width:600px; height: 350px; background: url(<?php echo "$forstaBilden"; ?>) center center; overflow: hidden; postion: relative;" /></a>
			</div>
		</div>
		<div class="gdl-column-item">
			<span class="left"><a class="fonts-com font-size-9" id="karta" href="http://api.eniro.com/qm-partnerintegrations/query?what=emappartner&partnername=homemap-show&x=<?php echo "$longitudeDot" ?>&y=<?php echo "$latitudeDot" ?>&wcmzoomlevel=16&tiletype=map&calloutcontent=<?php echo "$kommun" ?>&title=<?php echo "$adressRad1" ?>&popupOpen=true" rel="prettyPhoto[iframes]">visa karta</a> <a class="fonts-com font-size-9" href="<?php echo '/bildmall/?objid=' . $objID . '#planritning'; ?>">/ planritning</a></span>
			<span class="right"><a class="fonts-com font-size-9" href="<?php echo '/visa-bilder-hemnet.php?objekt=' . $objID; ?>">visa alla bilder</a></span>
		</div>
	</div> <!-- ten columns -->
	<div class="six columns gdl-last "> <!-- 360 -->
		<div class="gdl-column-item">
			<div class="gdl-image-frame shortcode-image-right"><a href="<?php echo '/visa-bilder-hemnet.php?objekt=' . $objID; ?>" title="Fantastic Frank Fastighetsmäkleri - <?php echo "$andraBildenNamn" ?>" ><img src="/wp-content/themes/incidental/images/icon/light/content-bg-transparent.png" style="border: none; width:350px; height: 170px; background: url(<?php echo "$andraBilden"; ?>) center center; overflow: hidden; postion: relative;" /></a>
			</div>
			<!-- "http://annstreetstudio.com/wp-content/uploads/2012/09/parislove9801.gif -->
		</div>
		<div class="gdl-column-item" style="margin-top: 4px;">
			<div class="gdl-image-frame shortcode-image-right"><a href="<?php echo '/visa-bilder-hemnet.php?objekt=' . $objID; ?>" title="Fantastic Frank Fastighetsmäkleri - <?php echo "$tredjeBildenNamn" ?>" ><img src="/wp-content/themes/incidental/images/icon/light/content-bg-transparent.png" style="border: none; width:350px; height: 170px; background: url(<?php echo "$tredjeBilden"; ?>) center center; overflow: hidden; postion: relative;" /></a>
			</div>
		</div>
	</div> <!-- six columns -->
</div> <!-- sixteen wrapper -->
...
<div class="sixteen columns wrapper">
	<div class="eight columns  gdl-first ">
		<div class="gdl-column-item">
		    ...
		    <!-- budgivning -->
		    <?php if (!empty($budTabell) && $kind != '510') { ?>
			<ul class="gdl-toggle-box">
				<li class="gdl-divider"><h5 class="toggle-box-head title-color gdl-title"><span class="toggle-box-head-image"></span>Visa budgivning</h5><div class="toggle-box-content"><?php echo "$budTabell"; ?></div></li>
			</ul>
			<?php } ?>
			
		    <?php if ($kind != '510') { ?>
		    <div><?php echo "$hemnet"; ?></div>
			<?php } ?>
			
		    <?php if ($kind == '510') { ?>
		    <p><strong>Vill ni veta slutpriset? Eller har en bostad på samma adress, kontakta oss...</strong></p>
...
<input type="hidden" name="meddelande" value="<?php echo "$adressRad1" . 'OBS SÅLD, länk: www.fantasticfrank.se/visa-bostad-hemnet.php?obj=' . $objID; ?>" size="200" />
</div>
...
<p><input type="submit" value="Skicka" class="wpcf7-form-control  wpcf7-submit svart" /></p>
	...
	<div class="four columns ">
		<div class="gdl-column-item">
				...
				<div class="shortcode-list shortcode-list-none">
					<ul>
						<?php if(($objekttyp == 0 || $objekttyp == 1 || $objekttyp == 4 || $objekttyp == 5 || $objekttyp == 6) && $upplatelseform !== '3') { ?>
						<!-- VILLA FRITIDSHUS MED MERA -->
							<?php if (strlen($objekttypText) > 0) { ?>
							<li><strong>Typ:</strong> <?php echo "$objekttypText"; ?></li> 
							<?php } ?>
							<?php if (strlen($boendeformtext) > 0) { ?>
							<li><strong>Kategori:</strong> <?php echo "$boendeformtext"; ?></li> 
							<?php } ?>
							<?php if (strlen($upplatelseformText) > 0) { ?>
							<li><strong>Ägandeform:</strong> <?php echo "$upplatelseformText"; ?></li> 
							<?php } ?>
							<?php if (strlen($tomtareal) > 0) { ?>
							<li><strong>Tomt:</strong> <?php echo "$tomtareal"; ?> m&sup2;</li> 
							<?php } ?>
							<?php if (strlen($byggAr) > 0) { ?>
							<li><strong>Byggår:</strong> <?php echo "$byggAr"; ?></li> 
							<?php } ?>
							<?php if (strlen($pantbrevSumma) > 0) { ?>
							<li><strong>Summa pantbrev:</strong> <?php echo "$pantbrevSummaText"; ?></li> 
							<?php } ?>
						<!-- LÄGENHET -->
						<?php } else { ?>
							<?php if (strlen($boendeformtext) > 0) { ?>
							<li><?php echo '<strong>' . "$boendeformtext" . '</strong>';
									  if (strlen($upplatelseformText) > 0) {
								      	echo ', ' . $upplatelseformText;
								      }
								 ?></li> 
							<?php } ?>
							<?php if (strlen($vaning) > 0) { ?>
							<li><?php echo '<strong>Våning</strong> ' . $vaning;
							          if (strlen($vaningAv) > 0) {
							          	echo ' av ' . $vaningAv . $hiss;
							          }
							  ?></li> 
							<?php } ?>
							<li><?php echo'<strong>Månadsavgift:</strong> ' . $avgift . ' ' . $avgiftText; ?></li>
						<?php } ?>
						<!-- ALLA -->
						<li><strong><?php echo $pristext; ?>: </strong><?php echo $formatPris; ?></li>
						<?php if(strlen($hogstaBud) > 0) { ?>
						<li><?php echo '<strong>Högsta bud </strong>' . ' ' . "$hogstaBud"; ?></li>
						<?php } ?>
					</ul>
				</div>
				<?php } ?>
				<?php if($kind != '510') { ?>				
					...
					<div class="shortcode-list shortcode-list-none">
						<ul>
					<?php if(!empty($visningLista) && $kind != '510') { ?>
							<?php echo "$visningLista" ?>
					<?php } else { ?> 
							<li>Gör en <a href="mailto:<?php echo "$maklareEmail"; ?>?subject=<?php echo "$adressRad1" ?>" class="underline">intresseanmälan</a> för visning.</li>
					<?php } ?>
						</ul>
					</div>
				<?php } ?>
				...
				<div class="shortcode-list shortcode-list-none">
					<ul>
						<li><div class="fb-like" data-href="<?php echo '/visa-bilder-hemnet.php?objekt=' . $objID; ?>" data-send="true" data-layout="button_count" data-width="200" data-show-faces="false"></div></li>
					</ul>
				</div>
				
				
				<?php if(!empty($lanklista)) { ?>			
	...
	<div class="four columns gdl-last" style="margin-right: -20px;">
		...
		<div class="gdl-column-item"> 			
			...
			<div style="float:right; margin-right: -20px;">
				<div class="gdl-image-frame" style="max-width: 100%; width: 220px;"><a href="<?php echo '/visa-bilder-hemnet.php?objekt=' . $objID; ?>" title="<?php echo "$maklareTitel"; ?> - <?php echo "$maklareNamn"; ?>" ><img src="<?php echo $maklareBildUrl . '&sizex=220'; ?>" style="width:220px;" alt="Visa alla bostadens bilder" /></a>
				</div>
				<div class="shortcode-list shortcode-list-none">
					<ul>
