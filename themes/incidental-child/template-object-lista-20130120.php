...
function get_kommande_under_hand( ) {
    ...
    while (($data = fgetcsv($handle, 1000, ",")) != FALSE) {
        $num = count($data);
        if ($row != 1 && $data[1] == 1) { /* skip first row */  
        	$gid        = $data[3];
		...
		$html .= '					<img class="hemnet-bild-1" src="/wp-content/themes/incidental/images/icon/light/content-bg-transparent.png" alt="' . $omrade . ', ' . $adress . '" title="kommer snart…"/>';
		$html .= '			</div>';
		$html .= '			<div class="gdl-image-frame hemnet-bild-frame-2" style="background: url( ' . $bildurl . ' ) no-repeat -130px center;">';
		$html .= '					<img class="hemnet-bild-2" src="/wp-content/themes/incidental/images/icon/light/content-bg-transparent.png" alt="' . $omrade . ', ' . $adress . '" title="kommer snart..."/>';
		$html .= '			</div>';
		$html .= '			<div class="gdl-image-frame hemnet-bild-frame-3" style="background: url( ' . $bildurl . ' ) no-repeat -265px center;">';
		$html .= '					<img class="hemnet-bild-3" src="/wp-content/themes/incidental/images/icon/light/content-bg-transparent.png" alt="' . $omrade . ', ' . $adress . '" title="kommer snart..."/>';
		$html .= '			</div>';
		$html .= '			<div class="gdl-image-frame hemnet-bild-frame-4" style="background: url( ' . $bildurl . ' ) no-repeat -400px center;">';
		$html .= '					<img class="hemnet-bild-4" src="/wp-content/themes/incidental/images/icon/light/content-bg-transparent.png" alt="' . $omrade . ', ' . $adress . '" title="kommer snart..."/>';
		$html .= '			</div>';
		$html .= '		</div>';
		$html .= '	</div>';
		...
		$html .= '	</div>';	
	  	...
	  	$objektListaKommande["$row"] = array("html" => "$html");
	    $html = '';
	    break;
      		} // switch
      } //if
...
//$html .= '<div id="blog-item-holder" class="blog-item-holder">';
  ...
  $objektLista 	   = vitec($xml);
  $objektListaKom  = get_kommande_under_hand( );
  $objektListaSald = vitec($xmlRef);
...
<?php
  foreach ($objektListaKom as $key=> $row) {
  	echo $row["html"];
					...
					<div class="sixteen columns gdl-first gdl-last" style="margin-bottom: 0px; ">
						<div class="gdl-column-item">
							<h5 class="title-color-kopa" style="margin-top: 10px;">urval av sålda bostäder</h5>
						</div>
					</div>
<?php
