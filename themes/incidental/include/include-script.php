<?php
	...
	function register_all_gdl_scripts(){
		...
		if( $GLOBALS['pagenow'] != 'wp-login.php' ){
			if(is_admin()){
				...
				wp_enqueue_style('style-custom', GOODLAYERS_PATH.'/style-custom.css');
	...
	*/ 
	...
	function register_non_admin_styles(){
		...
		if( $post->ID != 346 && $post->ID != 319 && $post->ID != 1148 && $post->ID != 401 ){ //346 är image sidan, 319 objekt 401 köpa
	...
	function register_non_admin_scripts(){
		...
		if( $gdl_is_responsive ){
			...
			if( $post->ID != 346 && $post->ID != 319 && $post->ID != 1148 && $post->ID != 401 ){ //346 är image sidan, 319 objekt
		...
		// Page post_type
			...
			// Köpa-sidan ska vara filterable
			if( $post->ID == 401 || $post->ID == 1251 ) {
				// /* MaKa */
