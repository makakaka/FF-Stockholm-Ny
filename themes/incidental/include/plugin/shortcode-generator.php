<?php
	...
	function gdl_gallery_shortcode( $atts, $content = null ){
		extract( shortcode_atts(array('title'=>'', 'width'=>'200', 'height'=>'200',	'row_num'=>'100', 'type'=>'', 'galid'=>''), $atts) );
		$gdl_gallery = "";
			...
			if( empty($type) ){ 
				foreach( $slider_xml_dom->documentElement->childNodes as $slider ){
					...
					if($current_num % $row_num == 0){
						...
						$gdl_gallery = $gdl_gallery . '<div class="gallery-thumbnail-image alignleft" style="margin-right: 10px; margin-left: 10px; margin-bottom: 20px;">';
					}
	...
	function gdl_tab_shortcode( $atts, $content = null ){
		...
		for($i=0; $i<$num; $i++){
			...
			$tab = $tab . '<li><a data-href="' . $tab_id  . '" class=" ';
		...
		for($i=0; $i<$num; $i++){
			...
			$tab = $tab . '<li data-href="' . $tab_id . '" class="';
	...
	function gdl_quote_shortcode( $atts, $content = null ){
		...
		return '<div class="shortcode-block-quote-' . $align . '" style="color:' . $color . '; border-color: ' . $color . ';">' . $content . '</div>';
