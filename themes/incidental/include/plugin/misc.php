	...
	*/
	...
	function print_nivo_slider($slider_xml, $size='980x360'){
		...
		for( $i=0; $i<$caption_num; $i++ ){
			...
			echo "<div class='gdl-slider-title gdl-title'>" . $caption_array[$i]['title'] . "</div>";
	...
	function print_flex_slider($slider_xml, $size="980x360"){
		...
		foreach($slider_xml->childNodes as $slider){
			...
			if( !empty($title) ){
				echo '<div class="flex-caption gdl-slider-caption"><div class="gdl-slider-title gdl-title">' . $title . '</div>' . $caption . '</div>'; 
			}
